...
struct MinMaxAgentPropVisitor {
    ...
    void visit(const Agent* a) {
        ...
        case TGAM:
            if(ah._drawTgam) a->visitProperties(*this); break;
        case TREG:
            if(ah._drawTgam) a->visitProperties(*this); break;
        case TCYT:
            if(ah._drawTgam) a->visitProperties(*this); break;
...
void AgentHistogram::updatePlot()
{
    ...
    pbar->setValue(50); pbar->repaint();
    {
        ...
        pbar->setValue(60); pbar->repaint();
        for(size_t i=0;i<sim.getTgamList().size();i++) {
            visitor.iter = 0;
            visitor.visit(&(sim.getTgamList()[i]));
        }
        pbar->setValue(70); pbar->repaint();
        for(size_t i=0;i<sim.getTregList().size();i++) {
            visitor.iter = 0;
            visitor.visit(&(sim.getTregList()[i]));
        }
        pbar->setValue(80); pbar->repaint();
        for(size_t i=0;i<sim.getTcytList().size();i++) {
            visitor.iter = 0;
            visitor.visit(&(sim.getTcytList()[i]));
        }
        pbar->setValue(90); pbar->repaint();
    }
    sim.unlock();
    ui->centralwidget->replot();
    pbar->setValue(100); pbar->repaint();
}
...
void AgentHistogram::setAgentFilter(int agentType, int state, int secondaryState, bool enabled) {
    ...
    case MAC: _macFilter[state][secondaryState] = enabled; break;
    case TGAM: _drawTgam = secondaryState == AgentsWidget::ENBL && enabled; break;
    case TCYT: _drawTcyt = secondaryState == AgentsWidget::ENBL && enabled; break;
    case TREG: _drawTreg = secondaryState == AgentsWidget::ENBL && enabled; break;
    default: break;
    }
    if(!isHidden())
        updatePlot();
