...
static void diffuse_u_nofluxbc(const Scalar* __restrict__ grid_u, Scalar* __restrict__ newgrid_u, const Pos& dim, const Scalar cutOff, const int* __restrict__ nCells)
{
...
// Use this for diffusion assuming no flux boundary conditions
////Assumes padding on borders
static void diffuse_v_nofluxbc(const Scalar* __restrict__ grid_v, Scalar* __restrict__ newgrid_v, const Pos& dim, const Scalar cutOff, const int* __restrict__ nCells)
{
...
void GrDiffusionADE_Swap::diffuse(GrSimulationGrid& grSim) const
{
  ...
  #pragma omp parallel sections //if(_threaded)   //Spawns a new team of threads
  {
    ...
    #pragma omp section
    {
      ::diffuse_u_nofluxbc(grid.u_INH(), nextGrid.u_INH(), grid.getRange(), _cutOffValue, grid.nCells());
      ::diffuse_v_nofluxbc(grid.v_INH(), nextGrid.v_INH(), grid.getRange(), _cutOffValue, grid.nCells());
