...
template<>
void oCSVStream::operator<<(const bool& data)
{
  (*f)<<data<<',';
}
template<>
void oCSVStream::operator<<(const unsigned& data)
{
  (*f)<<data<<',';
}

template<>
void oCSVStream::operator<<(const int& data)
{
  (*f)<<data<<',';
}

template<>
void oCSVStream::operator<<(const long& data)
{
  (*f)<<data<<',';
}

template<>
void oCSVStream::operator<<(const unsigned long& data)
{
  (*f)<<data<<',';
}
...
template<>
void oCSVStream::operator<<(const char& data)
{
  (*f)<<data<<',';
}
...
{
public:
  IntMtbStats(std::ostream* s, GrSimulation& sim) : oCSVStream(s)
  {
    outputHeader(sim);
  }
...
{
public:
  GeneralStats(std::ostream* s) : oCSVStream(s)
  {
    outputHeader();
  }
  void outputHeader()
  {
    ...
    write("NrSourcesTcyt");
    write("NrSourcesTreg");
    write("NrSourcesMacActivatedFree");
    write("NrSourcesTgamActivatedFree");
    write("NrSourcesTcytActivatedFree");
    write("NrSourcesTregActivatedFree");
    write("NrSourcesMacCrowded");
    write("NrSourcesTgamCrowded");
    write("NrSourcesTcytCrowded");
    write("NrSourcesTregCrowded");
    write("NrCaseated");
    write("MacApoptosisTNF");
    write("MrApoptTNF");
    write("MiApoptTNF");
    write("MciApoptTNF");
    write("MaApoptTNF");
  ...
  void saveRow(const GrSimulation& sim)
  {
    ...
    write(stats.getTotIntMtb());
    write(stats.getTotExtMtb());
    write(repExtMtb);
    ...
    write(stats.getTotkmRNA());
    write(stats.getTotIL10());
    write(stats.getTotCCL2());
    write(stats.getTotCCL5());
    write(stats.getTotCXCL9());

    Scalar lesionSize = 2 * sqrt((0.0004 * stats.getAreaCellDensity()) / PI);
    write(stats.getAreaTNF());
    write(stats.getAreaCellDensity());
    write(lesionSize);
    ...
    write(stats.getT8());
    write(stats.getTC());
    write(stats.getTH0lung());
    write(stats.getTH1lung());
    write(stats.getT80lung());
    write(stats.getT8lung());
    write(stats.getTClung());
...
class MolecularTrackingStats : public oCSVStream
{
  bool trackAll;
public:
  MolecularTrackingStats(std::ostream* s, bool _trackAll=false) : oCSVStream(s), trackAll(_trackAll)
  {
    outputHeader();
  }
  ...
  void outputHeader()
  {
    ...
    write("surfTNFR1");
    write("surfTNFR2");
    write("surfBoundTNFR1");
    write("surfBoundTNFR2");
    write("intBoundTNFR1");
    write("intBoundTNFR2");
    write("mTNFRNA");
    write("vTNFR1");
